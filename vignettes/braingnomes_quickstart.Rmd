...
This saves `project_config.yaml` in the project directory. For automation,
`initialize_project()` creates the directory layout and a disabled-stage
configuration without opening a prompt. Enable and fill in the stages you need
with `edit_project()` or by using the resulting YAML as a version-controlled
template.
...
## Step 2: Validate and run preflight checks

Reloading and validation are non-mutating: a malformed configuration is
reported but is never silently repaired or written. `doctor()` then checks the
submission environment, including scheduler commands, container compatibility,
storage permissions, stage-specific files, and SQLite tracking.
...
stopifnot(validation$valid, preflight$ok)
```

Pass `deep = TRUE` to also initialize the active reticulate Python and inspect
optional modules used by template-dependent postprocessing. Preflight never
submits jobs or changes project files.

## Step 3: Build and save an execution plan
...
their configuration and subject scope, making the reviewed request reusable.
...
## Step 4: Submit and observe the run

Submitting a plan returns a run handle with a stable run ID. That ID connects
tracking rows, logs, diagnosis, retry, and cancellation.
...
Direct `run_project()` calls remain supported for interactive use. They now
return the same run handle after submission, and `dry_run = TRUE` retains the
older submission-free preview.

## Step 5: Diagnose, retry, or cancel

```{r eval = FALSE}
diagnosis <- diagnose_project(scfg, run$run_id)

# Review the force-enabled retry plan before submitting it.
retry_plan <- retry_project_run(scfg, run$run_id, dry_run = TRUE)

# Preview scheduler cancellation commands before taking action.
cancel_project_run(scfg, run$run_id, dry_run = TRUE)
```

The interactive `diagnose_pipeline()` browser remains available when a guided
job-tree investigation is more useful than a scriptable report.

# Running BrainGnomes on the command line

BrainGnomes provides the same lifecycle without requiring an interactive R
session. A typical shell workflow is:
...
Retry and cancellation require `--dry-run` or the explicit `--yes` confirmation
flag. Use `BrainGnomes validate-bids /project/my_study` to submit the separately
